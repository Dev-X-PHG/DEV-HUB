...
Toggle = PVP:AddToggle("Toggle", {Title = "Auto Run Hack", Default = false })
Toggle:OnChanged(function(Value)
    getgenv().RunHack = Value
end)
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().RunHack then
            local CharacterPlayer = game.Players.LocalPlayer.Character
            if CharacterPlayer and CharacterPlayer:FindFirstChild("HumanoidRootPart") then
                getgenv().NoClip = true                
                CharacterPlayer.HumanoidRootPart.CFrame = CharacterPlayer.HumanoidRootPart.CFrame + Vector3.new(0, 50, 0)                
                repeat task.wait(0.1) 
                until not getgenv().RunHack
            end
        end
    end
end)
